        ...
        var uibModalInstance = $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.checkout = function() {
                    ...
                    console.log("Processing checkout with action URL:", actionUrl);
                    ...
                    then(function(response) {
                        ...
                        console.log("Checkout successful, invoice ID:", $scope.invoiceId);
                        ...
                        // Handle redirection or print receipt
                        if ($scope.shouldRedirectAfterCheckout === true) {
                            ...
                            console.log("Redirecting to invoice view:", $scope.invoiceId);
                            ...
                            setTimeout(function() {
                                ...
                                .then((value) => {
                                    if (value === "view") {
                                        console.log("User clicked View Invoice, redirecting to:", $scope.invoiceId);
                                        window.location = window.baseUrl + "admin/view_invoice.php?invoice_id=" + $scope.invoiceId;
                                    }
                                });
                            }, 100);
                ...
                $scope.checkoutWithFullPaidAndPrint = function() {
	                ...
	                window.swal({
					  title: "Full Payment!",
					  text: "Are you sure that you want to make full payment and view invoice?",
					  icon: "warning",
					  buttons: true,
					  dangerMode: false,
					})
					.then(function(willDelete) {
					    if (willDelete) {
		                    setTimeout(function() {
                                console.log("Processing full payment with redirect to view invoice");
                    ...
                    .then(function(willProceed) {
                        if (willProceed) {
                            ...
                            then(function(response) {
                                ...
                                // Reset POS
                                if (typeof $scope.$parent.resetPos === 'function') {
                                    $scope.$parent.resetPos(true);
                                }
                                
                                // Explicitly cancel any ongoing form submissions or pending redirects
                                if (window.event) {
                                    if (window.event.preventDefault) window.event.preventDefault();
                                    if (window.event.stopPropagation) window.event.stopPropagation();
                                }
                                
                                // Redirect to the invoice view page - use explicit path
                                console.log("Redirecting to:", window.baseUrl + "admin/view_invoice.php?invoice_id=" + invoiceId);
                                
                                // Use direct window.location assignment rather than href
                                setTimeout(function() {
                                    window.location = window.baseUrl + "admin/view_invoice.php?invoice_id=" + invoiceId;
                                }, 100);
