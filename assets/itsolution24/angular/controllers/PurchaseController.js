window.angularApp.controller("PurchaseController", [
    ...
    function (
        ...
        $("#invoice-invoice-list").dataTable({
            "xLanguage": { sProcessing: "<img src='../assets/itsolution24/img/loading2.gif'>" },
            "processing": true,
            "dom": "lfBrtip",
            "serverSide": true,
            ...
            "columnDefs": [
                { "targets": [0, 9, 10, 11, 12, 13], "orderable": false },
                { "className": "text-center", "targets": [0, 1, 4, 8, 9, 10, 11, 12] },
        ...
        // Initialize Angular watchers for GST and tax fields
        $scope.supplierGst = '';
        $scope.taxInformation = '';

        // Watch for changes to supplier GST
        $scope.$watch('supplierGst', function (newVal, oldVal) {
            if (newVal !== oldVal) {
                console.log("Supplier GST updated:", newVal);
            }
        });

        // Watch for changes to tax information
        $scope.$watch('taxInformation', function (newVal, oldVal) {
            if (newVal !== oldVal) {
                console.log("Tax Information updated:", newVal);
            }
        });

        // Watch for changes to order tax
        $scope.$watch('orderTax', function (newVal, oldVal) {
            if (newVal !== oldVal) {
                console.log("Order Tax updated:", newVal + "%");
            }
        });
        ...
        $(document).delegate("#create-purchase-submit", "click", function (e) {
            ...
            // Get GST and tax information from form fields
            var supplierGst = $("#supplier-gst").val();
            var taxInfo = $("#tax-info").val();
            console.log("Supplier GST:", supplierGst);
            console.log("Tax Information:", taxInfo);

            // Create FormData object to properly handle form submission
            var formData = new FormData(form[0]);
            
            // Ensure GST and tax fields are included
            formData.set('supplier_gst', supplierGst);
            formData.set('tax_information', taxInfo);
            ...
            $http({
                ...
                then(function (response) {
                    ...
                    if (response.data && response.data.errorMsg) {
                        alertMsg += "<p>" + response.data.errorMsg + "</p>";
        ...
        $scope.addOrderTax = function () {
            console.log("Order Tax updated:", $scope.orderTax + "%");
            $scope._calculateTotalPayable();
        };
        ...
        setTimeout(function () {
            $http({
                url: window.baseUrl + "_inc/ajax.php?csv=1&type=PURCHASECSVITEM",
                dataType: "json",
                method: "post",
                data: $.param({
                    sup_id: $scope.sup_id,
                    ...
                    $.map(data.data, function (item) {
                        setTimeout(function () {
                            var theItem = {
                                ...
                                itemCode: item.p_code,
                                categoryId: item.category_id,
                                itemQuantity: item.qty,
                                itemUnitName: item.unit_name,
        ...
        $("#pay-all").on("click", function (e) {
            ...
            var items = $(document).find("input[name*='select']:checked");
            var itemLength = items.length;
            if (itemLength <= 0) {
                Swal.fire("Warning!", 'Item not found!', "warning");
                return false;
            }
            ...
            window.swal({
                title: "Are You Sure?",
                text: "Pay To All Selected Invoices!",
                icon: "warning",
                buttons: {
                    cancel: true,
                    confirm: true,
                    ...
                    if (willDelete) {
                        ...
                        window.$.ajax({
                            ...
                            error: function (xhr, ajaxOptions, thrownError) {
                                ...
                                $(":input[type=\"button\"]").prop("disabled", false);
                                var alertMsg = "<div>";
                                window.angular.forEach(xhr.responseText, function (value) {
                                    alertMsg += "<p>" + value + ".</p>";
                                });
                                alertMsg += "</div>";
                                window.toastr.warning(alertMsg, "Warning!");
