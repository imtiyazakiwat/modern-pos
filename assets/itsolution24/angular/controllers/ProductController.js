    ...
    function (
        ...
        dt.dataTable({
            ...
            "columns": [
                ...
                { data: "p_image", orderable: false, className: "text-center" },
                { data: "p_code" },
                { data: "p_name", orderable: false },
                { data: "supplier", orderable: false },
                { data: "quantity_in_stock", className: "text-center" },
                { data: "purchase_price", className: "text-right" },
                { data: "sell_price", className: "text-right" },
                { data: "mrp", orderable: false, className: "text-center" },
                { data: "view_btn", orderable: false, className: "text-center" },
                { data: "edit_btn", orderable: false, className: "text-center" },
                { data: "purchase_btn", orderable: false, className: "text-center" },
                { data: "barcode_btn", orderable: false, className: "text-center" },
                { data: "delete_btn", orderable: false, className: "text-center" }
            ],
            "columnDefs": [
                { "visible": false, "targets": hideColumsArray }
            ],
            "createdRow": function (row, data, dataIndex) {
                var headers = dt.find('thead tr th');
                $(row).find('td').each(function (index) {
                    var headerText = headers.eq(index).text();
                    if (headerText) {
                        $(this).attr('data-title', headerText);
                    }
                });
            },
        ...
        // Stock adjustment
        $(document).delegate(".button-stock-adjustment", "click", function (e) {
            e.stopPropagation();
            e.preventDefault();
            StockAdjustmentModal(dt.DataTable().row($(this).closest("tr")).data());
        });
        ...
        $("#button-add-stock").on("click", function (e) {
            e.preventDefault();
            ...
            window.swal({
                title: "Are You Sure?",
                text: "Add Stock to All Selected Products!",
                icon: "warning",
                buttons: {
                    cancel: true,
                    confirm: true,
                ...
                .then(function (willDelete) {
                    ...
                    if (willDelete) {
                        var $btn = $tag.button("loading");
                        var qty = parseInt(window.prompt('Quantity', 0));
                        if (qty == null || qty == '' || qty <= 0) {
                            window.toastr.warning('Invalid Quantity', "Warning!");
                        } else {
                            $http({
                                url: window.baseUrl + "_inc/stock_adjustment.php?action_type=UPDATE",
                                method: "POST",
