        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                if (
                    ...
                    !$this->sendCommand(
                        'User & Password',
                        encode_data("\0" . $username . "\0" . $password),
                        235
                    )
                ) {
                    return false;
            ...
            case 'LOGIN':
                ...
                if (!$this->sendCommand('Username', encode_data($username), 334)) {
                    return false;
                }
                if (!$this->sendCommand('Password', encode_data($password), 235)) {
                    return false;
                }
                break;
            case 'CRAM-MD5':
                ...
                //Get the challenge
                $challenge = decode_data(substr($this->last_reply, 4));

                //Build the response
                $response = $username . ' ' . $this->hmac($challenge, $password);

                //send encoded credentials
                return $this->sendCommand('Username', encode_data($response), 235);
            case 'XOAUTH2':
