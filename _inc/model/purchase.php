...
class ModelPurchase extends Model 
{
	...
	{
		$statement = $this->db->prepare("SELECT SUM(`item_quantity`) as qty, SUM(`tax`) as tax, SUM(`item_total`) as total, GROUP_CONCAT(DISTINCT item_tax) AS item_tax, GROUP_CONCAT(DISTINCT `taxrates`.`taxrate_name`) AS taxrate_name, GROUP_CONCAT(DISTINCT `taxrates`.`code_name`) AS code_name, GROUP_CONCAT(DISTINCT `selling_item`.`taxrate_id`) AS taxrate_id FROM `purchase_item` LEFT JOIN `taxrates` ON (`purchase_item`.`taxrate_id` = `taxrates`.`taxrate_id`) WHERE invoice_id = ? GROUP BY `purchase_item`.`taxrate_id`");
