...
class ModelQuotation extends Model 
{
    public function createQuotation($request, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            $statement->execute(array($reference_no, $store_id, (int)$sup_id, (int)$category_id, (int)$brand_id,  (int)$product_id, $product_name, (float)$item_purchase_price, (float)$product_price, (float)$product_discount, $tax, (int)$taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id));
        }
        ...
        $statement->execute(array($reference_no, $store_id, (int)$customer_id, $customer_mobile, $quotation_note, (int)$total_items, $status, $payment_status, (int)$is_installment_order, (int)$user_id, $created_at));
    ...
    public function updateQuotation($reference_no, $request, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            $statement = $this->db->prepare("INSERT INTO `quotation_item` (reference_no, store_id, sup_id, category_id, brand_id, item_id, item_name, item_purchase_price, item_price, item_discount, item_tax, taxrate_id, tax, gst, cgst, sgst, igst, item_quantity, item_total, purchase_invoice_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($reference_no, $store_id, (int)$sup_id, (int)$category_id, (int)$brand_id, (int)$product_id, $product_name, $item_purchase_price, $product_price, $product_discount, $tax, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id));
        ...
        $statement->execute(array($customer_id, $customer_mobile, $quotation_note, $total_items, $status, $payment_status, (int)$is_installment_order, $reference_no, $store_id));
