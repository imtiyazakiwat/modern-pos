...
class ModelBankAccount extends Model 
{
	...
	{		
    	...
    	$statement->execute(array($data['account_name'], $data['account_details'],  $data['account_no'],$data['contact_person'], $data['phone_number'], $data['url'], date_time()));
	...
	public function getBankAccounts($data = array(), $store_id = null) 
	{
		$store_id = $store_id ? $store_id : store_id();
		$sql = "SELECT `ba`.`id`,
GROUP_CONCAT(DISTINCT `ba`.`account_name`) AS account_name,
GROUP_CONCAT(DISTINCT `ba`.`account_details`) AS account_details,
GROUP_CONCAT(DISTINCT `ba`.`initial_balance`) AS initial_balance,
GROUP_CONCAT(DISTINCT `ba`.`account_no`) AS account_no,
GROUP_CONCAT(DISTINCT `ba`.`contact_person`) AS contact_person,
GROUP_CONCAT(DISTINCT `ba`.`phone_number`) AS phone_number,
GROUP_CONCAT(DISTINCT `ba`.`opening_date`) AS opening_date,
GROUP_CONCAT(DISTINCT `ba`.`url`) AS url,
GROUP_CONCAT(DISTINCT `ba`.`total_deposit`) AS total_deposit,
GROUP_CONCAT(DISTINCT `ba`.`total_withdraw`) AS total_withdraw,
GROUP_CONCAT(DISTINCT `ba`.`total_transfer_from_other`) AS total_transfer_from_other,
GROUP_CONCAT(DISTINCT `ba`.`total_transfer_to_other`) AS total_transfer_to_other,
GROUP_CONCAT(DISTINCT `ba`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `ba`.`updated_at`) AS updated_at,
...
GROUP_CONCAT(DISTINCT `ba2s`,`sort_order`) AS sort_order
		FROM `bank_accounts` ba LEFT JOIN `bank_account_to_store` ba2s ON (`ba`.`id` = `ba2s`.`account_id`) WHERE `ba2s`.`store_id` = ? AND `ba2s`.`status` = ?";
