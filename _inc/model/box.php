...
class ModelBox extends Model 
{
	...
	public function editBox($box_id, $data) 
	{
    	...
    	$statement->execute(array($data['box_name'], $data['code_name'], $data['box_details'], (int)$box_id));
    	...
    	if (isset($data['box_store'])) {
			...
			foreach ($data['box_store'] as $store_id) {
			    ...
			    $statement->execute(array($store_id, (int)$box_id));
	...
	public function deleteBox($box_id) 
	{
    	...
    	$statement->execute(array((int)$box_id));
	...
	public function getBoxes($data = array(), $store_id = null) {
		...
		$sql = "SELECT `boxes`.`box_id`,
GROUP_CONCAT(DISTINCT `boxes`.`box_name`) AS box_name,
GROUP_CONCAT(DISTINCT `boxes`.`code_name`) AS code_name,
GROUP_CONCAT(DISTINCT `boxes`.`box_details`) AS box_details,
GROUP_CONCAT(DISTINCT `b2s`.`id`) AS id,
GROUP_CONCAT(DISTINCT `b2s`.`box_id`) AS box_id,
GROUP_CONCAT(DISTINCT `b2s`.`store_id`) AS store_id,
GROUP_CONCAT(DISTINCT `b2s`.`status`) AS status,
GROUP_CONCAT(DISTINCT `b2s`.`sort_order`) AS sort_orde 
		FROM `boxes` LEFT JOIN `box_to_store` b2s ON (`boxes`.`box_id` = `b2s`.`box_id`) WHERE `b2s`.`store_id` = ? AND `b2s`.`status` = ?";
