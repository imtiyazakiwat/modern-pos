...
class ModelSms extends Model 
{
	...
	{		
    	...
    	$statement->execute(array($data['schedule_datetime'], (int)$data['store_id'], $data['people_type'], $data['mobile_number'], $data['people_name'], $data['sms_text'], $data['campaign_name'], $data['process_status'], $data['total_try'], $data['created_at']));
	...
	public function getSMSGateway()
	{
		...
		return (new Sms\SmsGateway())->initGateway($activeGateway, sms_setting($activeGateway));
	}
}