...
class ModelInvoice extends Model
{
    public function putOrderOnHold($request, $ref_no, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            $statement->execute(array($ref_no, $store_id, (int)$product_id, (int)$category_id, (int)$brand_id, (int)$sup_id, $product_name, $product_price, $product_discount, $tax, $tax_method, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total));
        }
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            while ($quantity_exist > 0) {
                ...
                if ($purchase_item && $purchase_item['item_quantity'] <= $purchase_item['total_sell']) {
                    $statement = $this->db->prepare("UPDATE `purchase_item` SET `status` = ? WHERE `store_id` = ? AND `item_id` = ? AND `status` = ?");
            ...
            $statement->execute(array($invoice_id, $store_id, (int)$product_id, (int)$category_id, (int)$brand_id, (int)$sup_id, $product_name, $item_purchase_price, $product_price, $product_discount, $tax, $tax_method, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id, $installment_quantity));
    ...
    {
        $store_id = $store_id ? $store_id : store_id();
        $statement = $this->db->prepare("SELECT SUM(`item_quantity`) as qty, SUM(`tax`) as tax, GROUP_CONCAT(DISTINCT item_tax) AS item_tax, GROUP_CONCAT(DISTINCT `taxrates`.`taxrate_name`) AS taxrate_name, GROUP_CONCAT(DISTINCT `taxrates`.`code_name`) AS code_name, GROUP_CONCAT(DISTINCT `selling_item`.`taxrate_id`) AS taxrate_id FROM `selling_item` LEFT JOIN `taxrates` ON (`selling_item`.`taxrate_id` = `taxrates`.`taxrate_id`) WHERE `store_id` = ? AND invoice_id = ? GROUP BY `selling_item`.`taxrate_id`");
        $statement->execute(array($store_id, $invoice_id));
        return $statement->fetchAll(PDO::FETCH_ASSOC);
    }
