...
GROUP_CONCAT(DISTINCT `s2s`.`sort_order`) AS sort_order
		FROM `suppliers` s LEFT JOIN `supplier_to_store` s2s ON (`s`.`sup_id` = `s2s`.`sup_id`) WHERE `s2s`.`store_id` = ? AND `s2s`.`status` = ?";

		if (isset($data['filter_name'])) {
			$sql .= " AND `sup_name` LIKE '" . $data['filter_name'] . "%'";
		}

		$sql .= " GROUP BY `s`.`sup_id`";
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$sql = "SELECT SUM(`balance`) AS total, GROUP_CONCAT(DISTINCT store_id) AS store_id, GROUP_CONCAT(DISTINCT sup_id) AS sup_id FROM `supplier_to_store` s2s
		WHERE `store_id` = ?";
		if ($sup_id) {
