...
class ModelCategory extends Model 
{
	public function addCategory($data) 
	{
    	...
    	$statement->execute(array($data['category_name'], $data['category_slug'], (int)$data['parent_id'], $data['category_details'], $data['category_image'], date_time()));
	...
	public function getCategorys($data = array(), $store_id = null) {
		...
		$store_id = $store_id ? $store_id : store_id();
		$sql = "SELECT `categorys`.`category_id`,
...
GROUP_CONCAT(DISTINCT `c2s`.`sort_order`) AS sort_order
		FROM `categorys` LEFT JOIN `category_to_store` c2s ON (`categorys`.`category_id` = `c2s`.`ccategory_id`) WHERE `c2s`.`store_id` = ? AND `c2s`.`status` = ?";
