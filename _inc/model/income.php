...
class ModelIncome extends Model 
{
	...
	{	
		...
		$statement = $this->db->prepare("SELECT GROUP_CONCAT(DISTINCT `income_sources`.`for_sell`) AS for_sell, GROUP_CONCAT(DISTINCT `bank_transaction_price`.`amount`) AS `total` FROM `bank_transaction_info` 
			LEFT JOIN `income_sources` ON (`bank_transaction_info`.`source_id` = `income_sources`.`source_id`)
			LEFT JOIN `bank_transaction_price` ON (`bank_transaction_info`.`info_id` = `bank_transaction_price`.`info_id`)
		...
		foreach ($rows as $row) {
			if($row['for_sell'] == 1) {
	          $total += get_profit_amount($from,$to);
	        	...
	        	$total = (float)$total + (float)$row['total'];
	        }
		}
		return $total;
	}
