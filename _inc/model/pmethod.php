...
class ModelPmethod extends Model 
{
	...
	public function getPmethods($data = array(), $store_id = null) 
	{
		...
		$sql = "SELECT `pmethods`.`pmethod_id`,
GROUP_CONCAT(DISTINCT `pmethods`.`name`) AS name,
GROUP_CONCAT(DISTINCT `pmethods`.`code_name`) AS code_name,
GROUP_CONCAT(DISTINCT `pmethods`.`details`) AS details,
GROUP_CONCAT(DISTINCT `pmethods`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `pmethods`.`updated_at`) AS updated_at,
GROUP_CONCAT(DISTINCT `pay2s`.`p2s_id`) AS p2s_id,
GROUP_CONCAT(DISTINCT `pay2s`.`ppmethod_id`) AS ppmethod_id,
GROUP_CONCAT(DISTINCT `pay2s`.`store_id`) AS store_id,
GROUP_CONCAT(DISTINCT `pay2s`.`status`) AS status,
GROUP_CONCAT(DISTINCT `pay2s`.`sort_order`) AS sort_order
		FROM `pmethods` LEFT JOIN `pmethod_to_store` pay2s ON (`pmethods`.`pmethod_id` = `pay2s`.`ppmethod_id`) WHERE `pay2s`.`store_id` = ? AND `pay2s`.`status` = ?";
		...
		if (isset($data['only'])) {
			$sql .= " AND ``code_name` IN ('" . implode("','", $data['only']) . "'')";
		}

		if (isset($data['exclude'])) {
			$sql .= " AND `code_name` NOT IN ('".implode("','", $data['exclude'])."')";
		}
