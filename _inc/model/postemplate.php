...
class ModelPostemplate extends Model 
{
	...
	public function getTemplates($data = array(), $store_id = null) {
		...
		$sql = "SELECT `pos_templates`.`template_id`,
GROUP_CONCAT(DISTINCT `pos_templates`.`template_name`) AS template_name,
GROUP_CONCAT(DISTINCT `pos_templates`.`template_content`) AS template_content,
GROUP_CONCAT(DISTINCT `pos_templates`.`template_css`) AS template_css,
GROUP_CONCAT(DISTINCT `pos_templates`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `pos_templates`.`updated_at`) AS updated_at,
GROUP_CONCAT(DISTINCT `pos_templates`.`created_by`) AS created_by,
GROUP_CONCAT(DISTINCT `pt2s`.`pt2s`) AS pt2s,
GROUP_CONCAT(DISTINCT `pt2s`.`store_id`) AS store_id,
GROUP_CONCAT(DISTINCT `pt2s`.`ttemplate_id`) AS ttemplate_id,
GROUP_CONCAT(DISTINCT `pt2s`.`is_active`) AS is_active,
GROUP_CONCAT(DISTINCT `pt2s`.`status`) AS status,
GROUP_CONCAT(DISTINCT `pt2s`.`sort_order`) AS sort_order
		FROM `pos_templates` LEFT JOIN `pos_template_to_store` pt2s ON (`pos_templates`.`template_id` = `pt2s`.`ttemplate_id`) WHERE `pt2s`.`store_id` = ? AND `pt2s`.`status` = ?";
