...
class ModelBrand extends Model 
{
	...
	public function editBrand($brand_id, $data) 
	{
    	...
    	$statement->execute(array($data['brand_name'], $data['code_name'], $data['brand_details'], $data['brand_image'], (int)$brand_id));
    	...
    	if (isset($data['brand_store'])) 
    	{
			...
			if (!empty($store_ids)) {
				...
				foreach ($unwanted_stores as $store) {
					...
					$statement->execute(array($store_id, (int)$brand_id));
	...
	public function deleteBrand($brand_id) 
	{
    	...
    	$statement->execute(array((int)$brand_id));
        return $brand_id;
	}
...
GROUP_CONCAT(DISTINCT `brands`.`code_name`) AS code_name,
GROUP_CONCAT(DISTINCT `brands`.`brand_details`) AS brand_details,
GROUP_CONCAT(DISTINCT `brands`.`brand_image`) AS brand_image,
GROUP_CONCAT(DISTINCT `brands`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `brands`.`updated_at`) AS updated_at,
GROUP_CONCAT(DISTINCT `b2s`.`b2s_id`) AS b2s_id,
GROUP_CONCAT(DISTINCT `b2s`.`brand_id`) AS brand_id,
GROUP_CONCAT(DISTINCT `b2s`.`store_id`) AS store_id,
GROUP_CONCAT(DISTINCT `b2s`.`status`) AS status,
GROUP_CONCAT(DISTINCT `b2s`.`sort_order`) AS sort_order
		FROM `brands` LEFT JOIN `brand_to_store` b2s ON (`brands`.`brand_id` = `b2s`.`brand_id`) WHERE `b2s`.`store_id` = ? AND `b2s`.`status` = ?";
	...
	public function getSellingPrice($brand_id, $from, $to, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("SELECT SUM(`selling_price`.`discount_amount`) as discount, SUM(`selling_price`.`subtotal`) as total, GROUP_CONCAT(DISTINCT `selling_info`.`created_at`) AS created_at FROM `selling_info` 
	...
	public function getpurchasePrice($brand_id, $from, $to, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("SELECT SUM(`purchase_price`.`paid_amount`) as total, GROUP_CONCAT(DISTINCT `purchase_info`.`created_at`) AS created_at FROM `purchase_info` 
	...
	public function totalSell($brand_id, $from = null, $to = null, $store_id = null) 
	{
		...
		$statement = $this->db->prepare("SELECT SUM(`selling_item`.`item_total`) AS total, GROUP_CONCAT(DISTINCT `selling_info`.`created_at`) AS created_at FROM `selling_info` LEFT JOIN `selling_item` ON (`selling_info`.`invoice_id` = `selling_item`.`invoice_id`) WHERE $where_query GROUP BY `selling_item`.`brand_id`");
