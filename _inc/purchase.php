...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'CREATE')
{
  try {
    ...
    foreach ($request->post['products'] as  $product) {  
        ...
        $statement->execute(array($invoice_id, $store_id, $id, (int)$category_id, (int)$brand_id, $item_name, $item_purchase_price, $item_selling_price, $item_quantity, $status, $item_total, $item_tax, $tax_method, $taxrate, $taxrate, $cgst, $sgst, $igst));
...
if($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    $statement = db()->prepare("SELECT GROUP_CONCAT(DISTINCT id) AS id, GROUP_CONCAT(DISTINCT item_id) AS item_id, SUM(`item_quantity`) as item_quantity, SUM(`total_sell`) as total_sell, GROUP_CONCAT(DISTINCT status) AS status, GROUP_CONCAT(DISTINCT return_quantity) AS return_quantity FROM `purchase_item` WHERE `store_id` = ? AND `invoice_id` = ? GROUP BY `status` ORDER BY `id` DESC");
