<table class="table table-striped no-margin">
  ...
  <tbody>
    <?php foreach  (get_invoices('sell', store_id(), 5) as $row) : ?>
      <tr>
        <td class="text-center"><a ng-click="InvoiceViewModal({'invoice_id':'<?php echo $row['invoice_id'];?>'})" onClick="return false;" href="<?php echo root_url().ADMINDIRNAME;?>/view_invoice.php?invoice_id=<?php echo $row['invoice_id'];?>"><?php echo $row['invoice_id'];?></a></td>
