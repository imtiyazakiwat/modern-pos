    ...
    <?php if ($invoices = get_quotations(store_id(), 5)) : ?>
      <?php foreach ($invoices as $row) : ?>
        <tr>
          ...
          <td class="text-center"><a ng-click="QuotationViewModal({'reference_no':'<?php echo $row['reference_no'];?>'});" onClick="return false;" href="<?php echo root_url().ADMINDIRNAME;?>/quotation.php?reference_no=<?php echo $row['reference_no'];?>"><?php echo $row['reference_no'];?></a></td>
