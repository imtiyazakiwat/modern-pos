...
function generate_invoice_id($type = 'sell', $invoice_id = null)
{
    ...
    if ($invoice_model->hasInvoice($invoice_id)) {
        $invoice_id = my_str_replace(array('A','B','C','D','E','F','G','H','I','G','K','L','M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z'), '', $invoice_id);
...
{  
    ...
    if (isset($invoice['reference_no'])) {
        $reference_no = my_str_replace(array('A','B','C','D','E','F','G','H','I','G','K','L','M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z'), '', $reference_no);
...
function get_postemplate_data($invoice_id)
{
    ...
    $data = array(
      ...
      'paid_amount' => currency_format($invoice_info['paid_amount']+($invoice_info['balance']-$invoice_info['return_amount'])),
      'due' => currency_format($invoice_info['due']+$invoice_info['previous_due']),
      ...
      foreach ($payment as $key => $val) {
        if (in_array($key, array('sl'))) {
          $new_payment[$key] = $val;
            ...
            $o = '<b>'.$payment['name'].'</b>';
            $details = $payment['details'] ? valid_unserialize($payment['details']) : [];
            if (!empty($details)) {
                $o .= '<ul>';
                foreach ($details as $k => $val) {
                  $o .= '<li>'. my_str_replace('_',' ', utf8_strtoupper($k)) . ' = '.$val.'</li>';
                }
                $o .= '</ul>';
            }
            $new_payment[$key] = $o;
...
function get_postemplate_empty_data()
{
    $data = array(
      ...
      'footer_text' => '<strong>shreshtacollections.com</strong>',
