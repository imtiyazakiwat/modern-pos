...
GROUP_CONCAT(DISTINCT `purchase_info`.`brand_id`) AS brand_id,
GROUP_CONCAT(DISTINCT `purchase_info`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `purchase_info`.`updated_at`) AS updated_at,
GROUP_CONCAT(DISTINCT brands.brand_name) AS brand_name, GROUP_CONCAT(DISTINCT purchase_item.item_quantity) AS item_quantity, SUM(purchase_item.item_total) as purchase_price, SUM(purchase_item.item_quantity) as total_stock, SUM(purchase_price.paid_amount) as paid_amount FROM purchase_info 
