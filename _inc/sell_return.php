...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'RETURN')
{
  try {
    ...
    foreach ($items as $item) 
    {
      ...
      $statement = db()->prepare("INSERT INTO `return_items` SET `store_id` = ?, `reference_no` = ?, `item_id` = ?, `item_name` = ?, `item_quantity` = ?, `item_price` = ?, `item_purchase_price` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `item_total` = ?, `created_at` = ?");
      $statement->execute(array($store_id, $reference_no, $item_id, $item_name, $item_quantity, $item_price, $item_purchase_price, $item_tax, $cgst, $sgst, $igst, $item_total, date_time()));
    }
    ...
    $statement = db()->prepare("INSERT INTO `returns` SET `store_id` = ?, `reference_no` = ?, `invoice_id` = ?, `purchase_invoice_id` = ?, `customer_id` = ?, `note` = ?, `total_item` = ?, `total_quantity` = ?, `subtotal` = ?, `total_amount` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `total_purchase_price` = ?, `profit` = ?, `created_by` = ?, `created_at` = ?, `updated_at` = ?");
    $statement->execute(array($store_id, $reference_no, $invoice_id, $purchase_invoice_id, $customer_id, $note, $total_item, $total_quantity, $tsubtotal, $tpayable, $titem_tax, $tcgst, $tsgst, $tigst, $total_purchase_price, $profit, $user_id, date_time(), date_time()));
    ...
    if ($return_amount > 0) {
      ...
      // Add return amount in balance
      $statement = db()->prepare("UPDATE `customer_to_store` SET `balance` = `balance`+$return_amount WHERE `store_id` = ? AND `customer_id` = ?");
...
if($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'DELETE') 
{
  try {
    ...
    if (user_group_id() != 1 && !has_permission('access', 'delete_return')) {
    ...
    $Hooks->do_action('Before_Delete_Return', $request);

    $Hooks->do_action('After_Delete_Return', $currency);
    header('Content-Type: application/json');
...
if (isset($request->get['reference_no']) && $request->get['action_type'] == 'VIEW') 
{
    ...
    $invoice = $return_model->getInvoiceInfo($reference_no);
    $invoice_items = $return_model->getInvoiceItems($reference_no);
...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'customer_name' ,
      'formatter' => function($d, $row) {
        $customer_name = get_the_customer($row['customer_id'],'customer_name');
        return '<a href="customer.php?customer_id=' . $row['customer_id'] . '&customer_name='.$customer_name.'">' . $customer_name . '</a>';
      }
    ),
    array(
        'db' => 'reference_no',
    ...
    array(
        'db' => 'purchase_invoice_id',
        'dt' => 'purchase_invoice_id',
        'formatter' => function( $d, $row) {
    ...
    array(
        ...
        'dt' => 'items',
        'formatter' => function($d, $row) {
            return get_return_invoice_items_html($row['reference_no']);
        }
    ),
    array(
        'db' => 'created_by',
