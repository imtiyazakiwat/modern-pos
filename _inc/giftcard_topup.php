...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') {
  try {
    ...
    $topup_amount = (float)my_str_replace(',','',$request->post['amount']);
    ...
    if (($account_id = store('deposit_account_id')) && $topup_amount > 0) {
      ...
      $statement->execute(array($store_id, 1, (int)$account_id, (int)$source_id, (int)$exp_category_id, $ref_no, (int)$giftcard['id'], $transaction_type, $title, $details, $image, $user_id, date_time()));
