...
// DB table to use
$table = "(SELECT
GROUP_CONCAT(DISTINCT `customer_transactions`.`id`) AS id,
GROUP_CONCAT(DISTINCT `customer_transactions`.`type`) AS type,
GROUP_CONCAT(DISTINCT `customer_transactions`.`reference_no`) AS reference_no,
GROUP_CONCAT(DISTINCT `customer_transactions`.`ref_invoice_id`) AS ref_invoice_id,
GROUP_CONCAT(DISTINCT `customer_transactions`.`customer_id`) AS customer_id,
GROUP_CONCAT(DISTINCT `customer_transactions`.`store_id`) AS store_id,
GROUP_CONCAT(DISTINCT `customer_transactions`.`pmethod_id`) AS pmethod_id,
GROUP_CONCAT(DISTINCT `customer_transactions`.`notes`) AS notes,
GROUP_CONCAT(DISTINCT `customer_transactions`.`amount`) AS amount,
GROUP_CONCAT(DISTINCT `customer_transactions`.`balance`) AS balance,
GROUP_CONCAT(DISTINCT `customer_transactions`.`created_by`) AS created_by,
GROUP_CONCAT(DISTINCT `customer_transactions`.`created_at`) AS created_at,
GROUP_CONCAT(DISTINCT `customer_transactions`.`updated_at`) AS updated_at
FROM customer_transactions 
  WHERE $where_query GROUP BY id ORDER BY id DESC
