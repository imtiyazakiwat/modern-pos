...
// DB table to use
$table = "(SELECT @sl:=@sl+1 AS sl, GROUP_CONCAT(DISTINCT selling_info.invoice_id) as invoice_id, GROUP_CONCAT(DISTINCT selling_info.created_at) as created_at, GROUP_CONCAT(DISTINCT selling_item.id) as id, GROUP_CONCAT(DISTINCT selling_item.item_id) as item_id, GROUP_CONCAT(DISTINCT selling_item.item_name) as item_name, SUM(selling_item.item_quantity) as total_item, SUM(selling_item.item_discount) as discount, SUM(selling_item.item_tax) as tax, SUM(selling_item.item_purchase_price) as purchase_price, SUM(selling_item.item_total) as sell_price FROM selling_item 
