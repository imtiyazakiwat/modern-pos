...
class Email {
	...
	public function initialize(array $config = array())
	{
		...
		$this->charset = utf8_strtoupper($this->charset);
	...
	{
		$this->_body = my_rtrim(my_str_replace("\r", '', $body));
	...
	public function attach($file, $disposition = '', $newname = NULL, $mime = '')
	{
		...
		$this->_attachments[] = array(
			...
			'type'		=> $mime,
			'content'	=> chunk_split(encode_data($file_content)),
			'multipart'	=> 'mixed'
		);
	...
	{
		$this->_headers[$header] = my_str_replace(array("\n", "\r"), '', $value);
		return $this;
	}
	...
	{
		$from = my_str_replace(array('>', '<'), '', $this->_headers['Return-Path']);
	...
	protected function _get_alt_message()
	{
		...
		$body = my_str_replace("\t", '', preg_replace('#<!--(.*)--\>#', '', my_trim(strip_tags($body))));

		for ($i = 20; $i >= 3; $i--)
		{
			$body = my_str_replace(str_repeat("\n", $i), "\n\n", $body);
		}
	...
	protected function _write_headers()
	{
		...
		if ($this->_get_protocol() === 'mail')
		{
			$this->_header_str = my_rtrim($this->_header_str);
		}
	}
	...
	protected function _prep_quoted_printable($str)
	{
		...
		$str = my_str_replace(array('{unwrap}', '{/unwrap}'), '', $str);
		...
		foreach (explode("\n", $str) as $line)
		{
			...
			for ($i = 0; $i < $length; $i++)
			{
				...
				elseif ($ascii === 61)
				{
					$char = $escape.utf8_strtoupper(sprintf('%02s', dechex($ascii)));  // =3D
				}
				elseif ( ! in_array($ascii, $ascii_safe_chars, TRUE))
				{
					$char = $escape.utf8_strtoupper(sprintf('%02s', dechex($ascii)));
				}
	...
	protected function _prep_q_encoding($str)
	{
		...
		for ($i = 0, $length = self::strlen($output); $i < $chars; $i++)
		{
			$chr = ($this->charset === 'UTF-8' && ICONV_ENABLED === TRUE)
				? '='.implode('=', str_split(utf8_strtoupper(bin2hex(iconv_substr($str, $i, 1, $this->charset))), 2))
				: '='.utf8_strtoupper(bin2hex($str[$i]));
	...
	protected function _remove_nl_callback($matches)
	{
		...
		{
			$matches[1] = my_str_replace(array("\r\n", "\r", "\n"), '', $matches[1]);
		}
	...
	protected function _smtp_authenticate()
	{
		...
		$this->_send_data(encode_data($this->smtp_user));
		...
		$this->_send_data(encode_data($this->smtp_pass));
	...
	{
		$this->_debug_msg[] = my_str_replace('%s', $val, $msg).'<br />';
	}
	...
	protected static function substr($str, $start, $length = NULL)
	{
		...
		{
			// my_mb_substr($str, $start, null, '8bit') returns an empty
			// string on PHP 5.3
			isset($length) OR $length = ($start >= 0 ? self::strlen($str) - $start : -$start);
			return my_mb_substr($str, $start, $length, '8bit');
		}
