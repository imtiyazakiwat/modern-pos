...
class Language 
{
	...
	public function get($key) 
	{
		...
		$key1 = my_str_replace(array('_', 'menu', 'label', 'text', 'button', 'title', 'success', 'hint', 'placeholder'), ' ', $key);
	...
	public function load() 
	{
		if (empty($this->data)) {
			...
			foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $value) {
				if ($value && $value['lang_key'] && $value['lang_value']) {
					$this->data[trim($value['lang_key'])] = my_trim($value['lang_value']);
				}
			}
		}
	}
