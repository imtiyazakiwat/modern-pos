...
class Cart {
	...
	protected function _insert($items = array())
	{
		...
		// Our solution is to convert the options array to a string and sha256 it along with the product ID.
		// This becomes the unique "row ID"
		if (isset($items['options']) && count($items['options']) > 0)
		{
			$rowid = hash('sha256',$items['id'].serialize($items['options']));
		}
		else
		{
			// No options were submitted so we simply sha256 the product ID.
			// Technically, we don't need to sha256 the ID in this case, but it makes
			// sense to standardize the format of array indexes for both conditions
			$rowid = hash('sha256',$items['id']);
		}
