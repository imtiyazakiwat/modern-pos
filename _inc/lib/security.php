...
class Security {
	...
	public function __construct($charset = 'UTF-8')
	{
		...
		if (store('csrf_protection') && ! is_cli())
		{
			// CSRF config
			foreach (array('csrf_expire', 'csrf_token_name', 'csrf_cookie_name') as $key)
			{
				if (NULL !== ($val = store($key)))
				{
					$this->{'_'.$key} = $val;
				}
			}
			...
			if ($cookie_prefix = store('cookie_prefix'))
			{
	...
	public function csrf_set_cookie()
	{
		...
		$secure_cookie = (bool) store('cookie_secure');
	...
	public function xss_clean($str, $is_image = false)
	{
		...
		// See https://github.com/bcit-ci/Phspark/issues/5420
		$str = preg_replace(
			'#(alert|prompt|confirm|cmd|passthru|eval|exec|expression|system|fopen|fsockopen|file|file_get_contents|readfile|unlink)(\s*)`(.*?)`#si',
			'\\1\\2&#96;\\3&#96;',
			$str
	 ...
	 *
	 * @see		https://github.com/bcit-ci/Phspark/issues/4877
	 * @param	array	$matches
	 * @return	string
	 */
	...
	protected function _sanitize_naughty_html($matches)
	{
		static $naughty_tags    = array(
			...
			'plaintext', 'xstyle', 'script', 'textarea', 'title', 'math', 'video', 'svg', 'xml', 'xss'
		);

		static $evil_attributes = array(
			'on\w+', 'xstyle', 'xmlns', 'formaction', 'form', 'xlink:href', 'FSCommand', 'seekSegmentTime'
		);
