...
class Mobiledetect
{
    ...
    public function getHttpHeader($header)
    {
        ...
        if (strpos($header, '_') === false) {
            $header = my_str_replace('-', '_', $header);
            $header = utf8_strtoupper($header);
        }
    ...
    public function setCfHeaders($cfHeaders = null) {
        ...
        foreach ($cfHeaders as $key => $value) {
            if (substr(strtolower($key), 0, 16) === 'http_cloudfront_') {
                $this->cloudfrontHeaders[utf8_strtoupper($key)] = $value;
                $response = true;
            }
        }
     ...
     *
     * @todo Remove the error supression from my_str_replace() call.
     *
    ...
    {
        $ver = my_str_replace(array('_', ' ', '/'), '.', $ver);
        $arrVer = explode('.', $ver, 2);

        if (isset($arrVer[1])) {
            $arrVer[1] = my_str_replace('.', '', $arrVer[1]); // @todo: treat strings versions.
        }
    ...
    public function version($propertyName, $type = self::VERSION_TYPE_STRING)
    {
        ...
        if (true === isset($properties[$propertyName])) {
            ...
            foreach ($properties[$propertyName] as $propertyMatchString) {
                ...
                $propertyPattern = my_str_replace('[VER]', self::VER, $propertyMatchString);
