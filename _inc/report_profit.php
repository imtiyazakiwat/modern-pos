...
GROUP_CONCAT(DISTINCT income_sources.source_slug) AS source_slug, GROUP_CONCAT(DISTINCT income_sources.for_sell) AS for_sell, GROUP_CONCAT(DISTINCT income_sources.for_due_collection) AS for_due_collection, GROUP_CONCAT(DISTINCT income_sources.profitable) AS profitable, GROUP_CONCAT(DISTINCT bank_transaction_price.price_id) AS price_id, SUM(bank_transaction_price.amount) amount 
  ...
  WHERE $where_query GROUP BY transaction_type) as bank_transaction_info";
