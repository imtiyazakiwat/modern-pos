...
// Very early debug - this should always execute
error_log("DEBUG: barcode_print.php - Script started");
file_put_contents('barcode_debug.log', "DEBUG: barcode_print.php - Script started\n", FILE_APPEND);

ob_start();
session_start();

error_log("DEBUG: barcode_print.php - About to include _init.php");
file_put_contents('barcode_debug.log', "DEBUG: barcode_print.php - About to include _init.php\n", FILE_APPEND);

include ("../_init.php");

error_log("DEBUG: barcode_print.php - _init.php included successfully");
file_put_contents('barcode_debug.log', "DEBUG: barcode_print.php - _init.php included successfully\n", FILE_APPEND);

// Debug: Log POST request
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    error_log("DEBUG: POST request received to barcode_print.php");
    error_log("DEBUG: POST data: " . print_r($_POST, true));
    file_put_contents('barcode_debug.log', "DEBUG: POST request received to barcode_print.php\n", FILE_APPEND);
    file_put_contents('barcode_debug.log', "DEBUG: POST data: " . print_r($_POST, true) . "\n", FILE_APPEND);
}
          ...
          <div class='box-body'> 
            <form id="form-barcode-generate" class="form-horizontal" action="barcode_print.php#barcode-con" method="post" target="_self">
              ...
              if(isset($request->post['products'])):
                error_log("DEBUG: Products found in POST data");
                file_put_contents('barcode_debug.log', "DEBUG: Products found in POST data\n", FILE_APPEND);
                $per_page = $request->post['per_page'];
                error_log("DEBUG: Per page value: " . $per_page);
                file_put_contents('barcode_debug.log', "DEBUG: Per page value: " . $per_page . "\n", FILE_APPEND);
                if (!$per_page) {
                  error_log("DEBUG: No per_page value, redirecting");
                  file_put_contents('barcode_debug.log', "DEBUG: No per_page value, redirecting\n", FILE_APPEND);
                ...
                // Barcode
                error_log("DEBUG: Creating barcode generator");
                file_put_contents('barcode_debug.log', "DEBUG: Creating barcode generator\n", FILE_APPEND);
                try {
                  $generator = barcode_generator();
                  error_log("DEBUG: Barcode generator created successfully");
                  file_put_contents('barcode_debug.log', "DEBUG: Barcode generator created successfully\n", FILE_APPEND);
                } catch (Exception $e) {
                  error_log("DEBUG: Error creating barcode generator: " . $e->getMessage());
                  file_put_contents('barcode_debug.log', "DEBUG: Error creating barcode generator: " . $e->getMessage() . "\n", FILE_APPEND);
                  echo "<div class='alert alert-danger'>Error creating barcode generator: " . $e->getMessage() . "</div>";
                  exit;
                }
