...
$order_printer_ids = store('order_printers') ? json_decode(store('order_printers')) : array();
...
<head>
	...
	<script type="text/javascript">
        "use strict";
		var baseUrl = "<?php echo my_trim(root_url(),'/'); ?>";
		var lang = "<?php echo $active_lang;?>";
		var timezone = "<?php echo get_preference('timezone');?>";
		var adminDir = "<?php echo ADMINDIRNAME; ?>";
		var user = <?php echo json_encode(array_diff_key(get_the_user(user_id()), array_flip(array('password','pass_reset_code','reset_code_time')))); ?>;
...
<?php include('../_inc/template/pos_skeleton.php'); ?>
						...
						<div class="contents">
							...
							<div id="item-list">
								<div ng-show="showLoader" class="ajax-loader">
									<img src="../assets/itsolution24/img/loading2.gif" alt="loading imgage">
								</div>
								<div class="add-new-product-wrapper" data-ng-class="{'show': showAddProductBtn}">
									<div class="add-new-product">
										<div class="add-new-product-btn">
											<button ng-click="createNewProduct()" class="btn btn-lg btn-danger w-auto">
												<span class="fa fa-fw fa-plus"></span>
												<span><?php echo trans('button_add_product'); ?></span>
											</button>
											<a ng-click="OpenPurchaseProductModal();" class="btn btn-lg btn-danger w-auto" >
								...
								<div ng-repeat="products in productArray" id="{{ $index }}" class="btn btn-flat item">
									<div ng-click="addItemToInvoice(products.p_id,products)" class="item-inner">
										...
										<span class="item-info" data-id="{{ products.p_id }}" data-name="{{ products.p_name }}">
											<span>
												{{ products.p_name | cut:true:20:' ...' }} ({{ products.quantity_in_stock | formatDecimal }})
											</span>
										</span>
							...
							<div class="well well-sm">
								...
								<div id="people-area">
									...
									<div class="edit-icon pointer ">
										...
										<span id="add-customer-mobile-number-handler" class="fa fa-mobile"></span>
											...
											<tbody>
												<tr ng-repeat="items in itemArray" class="invoice-item">
													<td class="product-quantity" id="invoice-item-{{ items.id }}">
														...
														<input type="text" name="item_price_{{ items.id }}" class="item_quantity text-center" id="item_quantity_{{ items.id }}" value="{{ items.quantity }}" data-itemid="{{ items.id }}" onClick="this.select();" ondrop="return false;" onpaste="return false;">
														<span class="fs-12"><i>{{ items.unitName }}</i></span>
													...
													<td class="product-price">
														<?php if (get_preference('change_item_price_while_billing') == 1) : ?>
															<input type="text" class="text-center item_price" id="item_price_{{ items.id }}" name="item_price_{{ items.id }}" value="{{ items.price | formatDecimal:2 }}" data-itemid="{{ items.id }}" onClick="this.select();" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;">
														<?php else : ?>
    ...
    <aside class="scrolling-sidebar scrolling-sidebar-dark">
        ...
        ?>
        <div class="searchbox text-center searchbox-m">
          <input ng-model="searchList" class="form-control r-50" type="search" name="search" placeholder="<?php echo trans('placeholder_search_here');?>">
        </div>
        <ul filter-list="searchList" class="list-group searchlist-p">
        <?php $inc=0;foreach ($shortcut_links as $link) : $btnColor=$inc % 2 == 0 ? 'success' : 'success'?>
          <?php if (user_group_id() == 1 || has_permission('access', $link['permission_slug'])) :?>
            <li class="list-group-item p-2">
                <a class="btn btn-<?php echo $btnColor;?> btn-block search-item" href="<?php echo root_url().$link['href'];?>"><span class="fa fa-fw <?php echo $link['icon'];?>"></span> <?php echo $link['title'];?></a>
            </li>
          <?php endif;?>
        <?php $inc++;endforeach;?>
        </ul>                    
